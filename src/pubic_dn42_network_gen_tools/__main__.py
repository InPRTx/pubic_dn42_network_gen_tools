...
class NetWorkTools:
    ...
    def gen_bird_ibgp(self):
        ...
        for peer_name in config.ibgp_network.pub:
            if node_name not in config.ibgp_network.pub or peer_name == node_name or peer_name in filenames:
                continue
            print(peer_name)
...
neighbor {config.node.get(peer_name).ipv6_pub.address.compressed} as OWNAS;
            ...
            if not is_develop:
                file_path = host_mode_file_path(f'/etc/bird/ibgps/pub_{peer_name}.conf')
