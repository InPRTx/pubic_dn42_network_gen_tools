...
class NetWorkTools:
    def __gen_bird_ibgp_conf(self, peer_name: str, is_rr_client: bool = False):
        # 插入中间自定义配置
        str_rr_client = '\n    rr client;' if is_rr_client else ''
    ...
    neighbor {config.node.get(peer_name).ipv6_pub.address.compressed} as OWNAS;{str_rr_client}
}}"""
        if not is_develop:
            file_path = host_mode_file_path(f'/etc/bird/ibgps/{peer_name}.conf')
            open(file_path, 'w').write(bird_ibgp_conf)
            logging.info(f'写入{file_path}完成')
        else:
            logging.info(f"打印配置为 {peer_name} {bird_ibgp_conf}", )
    ...
    def gen_bird_ibgp(self):
        for peer_name in config.ibgp_network.pub:
            if node_name not in config.ibgp_network.pub or peer_name == node_name:
                continue
            self.__gen_bird_ibgp_conf(peer_name)
        for peer_name, nodes_name in config.ibgp_network.rr.items():
            # rr主节点
            if node_name == peer_name:
                for peer_name2 in nodes_name:
                    if peer_name2 == node_name:
                        continue
                    self.__gen_bird_ibgp_conf(peer_name2, True)
            elif node_name in nodes_name:
                self.__gen_bird_ibgp_conf(peer_name, False)
            # print(peer_name)
        for peer_name, nodes_name in config.ibgp_network.rr_mutual.items():
            # rr主节点
            if node_name == peer_name:
                for peer_name2 in nodes_name:
                    if peer_name2 == node_name:
                ...
                self.__gen_bird_ibgp_conf(peer_name, True)
            print(peer_name)
...
class NetworkOSPF:
    ...
    async def ping_mtu(self, __test_ip: ipaddress.IPv4Address | ipaddress.IPv6Address,
        ...
        if get_ping_result and get_ping_result.cost != 100:
            ...
            max_mtu = 1440 if 5 < len(interface_name) and interface_name[3:5] == 'i4' else 1420
            test_mtus = list(range(max_mtu, 1324, -8))  # 1328
    ...
    def gen_ospf_config(self, ospf_interface_str: str = ''):
        ping_results: List[PingResult] = asyncio.run(self.__test_all_ip('mtu'))
        for ping_result in ping_results:
            cost = 1 if ping_result.cost == 0 else ping_result.cost  # 修复cost为0的情况
            ospf_interface_str += f'\n        interface "{ping_result.interface_name}" {{type ptp; cost {cost}; }}; # {ping_result.text}'
...
if __name__ == '__main__':
    ...
    parser.add_argument("module", type=str, choices=[
        "gen_bird_head", "gen_systemd_networkd", "gen_bird_ibgp",
        "gen_iptables", "gen_wg", "gen_gre", "gen_mtu",
        "remove_gre", "gen_ospf_cost"
    ])
    args = parser.parse_args()

    actions = {
        'gen_bird_head': (BirdHeadGen, 'gen_bird_head'),
        'gen_systemd_networkd': (BirdHeadGen, 'gen_systemd_networkd'),
        'gen_iptables': (BirdHeadGen, 'gen_network_iptables'),
        'gen_bird_ibgp': (NetWorkTools, 'gen_bird_ibgp'),
        'gen_wg': (NetWorkWG, 'gen_wg_conf'),
        'gen_gre': (NetWorkTools, 'gen_host_gre_cmd2'),
        'gen_mtu': (NetworkOSPF, 'gen_mtu'),
        'remove_gre': (NetWorkTools, 'remove_lxc_gre'),
        'gen_ospf_cost': (NetworkOSPF, 'gen_ospf_config')
    }

    if args.module in actions:
        cls, method = actions[args.module]
        instance = cls()
